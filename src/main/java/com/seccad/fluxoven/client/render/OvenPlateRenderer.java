...
public class OvenPlateRenderer implements BlockEntityRenderer<OvenPlateTileEntity> {
    ...
    @Override
    public void render(OvenPlateTileEntity tile, float partialTicks, @Nonnull PoseStack matrixStack,
        ...
        if (tile.isWorking()) {
            this.renderPlateSurface(matrixStack, buffer, facing, combinedLight);
        }
        ...
        for (int i = 0; i < 4; i++) {
            ...
            matrixStack.translate(0.5D, 0.135D, 0.5D);
            ...
            matrixStack.translate(xBase + xShift, 0, zOffset);

            this.drawItemStack(matrixStack, buffer, stack, combinedLight, combinedOverlay);

            if (showNumbers) {
                this.renderStackCount(String.valueOf(stack.getCount()), matrixStack, buffer, combinedLight, facing);
            }
    ...
    private void drawItemStack(PoseStack matrixStack, MultiBufferSource buffer, ItemStack stack, int light, int overlay) {
        matrixStack.pushPose();
        ...
        for (int layer = 0; layer < renderCount; layer++) {
            ...
            matrixStack.translate(0, layer * 0.015D, 0);
    ...
    private void renderStackCount(String text, PoseStack matrixStack, MultiBufferSource buffer, int light, Direction facing) {
        matrixStack.pushPose();

        matrixStack.translate(0, 0.22D, 0);

        matrixStack.mulPose(Vector3f.YP.rotationDegrees(facing.toYRot()));
        matrixStack.mulPose(this.context.getBlockEntityRenderDispatcher().camera.rotation());
        ...
        font.drawInBatch(text, xOffset, 0, 0xFFFFFF, true, matrixStack.last().pose(), buffer, false, 0, light);

        matrixStack.popPose();
    }

    private void renderPlateSurface(PoseStack matrixStack, MultiBufferSource buffer, Direction facing, int light) {
        matrixStack.pushPose();

        matrixStack.translate(0.5, 0.1265, 0.5);
        matrixStack.mulPose(Vector3f.YP.rotationDegrees(-facing.toYRot()));
